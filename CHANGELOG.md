...
## [Unreleased]
### Added
- `tests/negatives/worker-stall-falsepositive.sh` — mutation-checked: a healthy, slow, silent worker must survive the default config; a genuinely wedged worker must still die by the wall clock; a supervisor kill must never masquerade as a task failure. (#70)
### Fixed
- **`spec-conform.py`: a port declaring `__init__` could NEVER conform, and a `TypedDict` in the port was treated as an implementation obligation** — so every *realistic* port (which has both) failed conformance forever, training users to disable the hook and restoring the decorative-port state #57 fixed. #57's own tests missed it because their fixtures had neither. Fixed: never skip a dunder the class actually defines; treat a `TypedDict`/`Protocol`/`NamedTuple`/`Enum`/annotations-only class as a *type declaration*, not an interface to implement. `tests/negatives/spec-conform.sh` now asserts a realistic port (with `__init__` + a `TypedDict`) conforms when it should and is caught when it drifts — mutation-checked. (#69)
- **The stall detector KILLED HEALTHY WORKERS.** It measured growth of the worker's stdout+stderr — but the worker runs with `--output-format json`, which prints the **entire result at the end, in one blob**. A healthy worker therefore emits **zero incremental output** for its whole run, so *"quiet"* was **always true** and `FLEET_WORKER_STALL_S` was never a liveness signal: it was a hidden **second wall-clock timeout** that killed any worker slower than 600s. It corrupted a live experiment (#61) — healthy `api` workers were killed at exactly 600s, and because the arm under test had a longer prompt (so ran slower), the false positive hit **that arm harder**, biasing the result *toward the hypothesis*. **A monitor whose input is constant is worse than no monitor: it fires on the wrong thing and you believe it.** Stall detection now defaults to **0 (off)** and is only meaningful once the worker streams; the **wall-clock timeout** is the real protection against a wedged worker and needs no output at all. And a supervisor kill is now reported **distinctly from a task failure** (`WALL-TIMEOUT` / `STALL-KILL`, both saying *"this is NOT a task failure"*) — otherwise a CI run or an experiment silently counts infrastructure as a result, which is exactly what happened. (#70)
- `experiments/stale-port/` — the #61 adversarial arm: **what does an ENFORCED but STALE contract cost?** n=6/arm, GLM-5.2. **The literature's −97% "confidently wrong" catastrophe did NOT fire: 0/12 trials shipped code that passed the conformance gate but failed the oracle.** A stale enforced contract instead cost THROUGHPUT — the agent **escalated** (4/6: refused and asked for `fleet spec amend`, the path #57's own error text prescribes) or **hedged** (2/6: built both interfaces, shipped correctly). It degraded fail-safe, not catastrophic — but at ~2× time and ~2× cost, and **prose alone scored 6/6**, so this removes the strongest argument AGAINST typed ports without supplying one FOR them. The decisive prose-vs-typed-at-scale question (#61) is still open. Found and fixed #69 and #70 in the process. (#61)
...
- `tests/negatives/spec-conform.sh` — mutation-checked: making the check always pass fails the drift assertion; removing the no-hook warning fails the anti-silent-failure assertion. (#57)
...
### Fixed
- **`fleet integrate` now supports `--cohort`: a set of branches merged TOGETHER and gated ONCE.** The per-branch gate rolled back any branch that was not *independently* green — but `fanout` (especially with `provides`/`consumes` ports) **deliberately produces units that are only green together**: a consumer needs its provider, a provider is useless alone. So if your `fleet_gate` runs an integration test — which is exactly what fleet tells you the oracle should be — merging `store` alone failed (no `api`), merging `api` alone failed (no `store`), **both were rejected and rolled back, and the FINAL gate ran on a tree containing neither.** Both units were perfectly good; the gate was unsatisfiable *by construction*. Found by the #50 experiment, which it silently emptied in all 6 trials. A cohort is atomic: a red combined gate rolls back the **whole** cohort, never a member; a merge conflict inside a cohort is reported per-branch and never masquerades as a gate verdict; the per-branch gate still runs as a labelled **diagnostic hint** that vetoes nothing. `fanout` now prints the exact `fleet integrate --cohort …` line when the manifest declares any port edge — a port edge *is* a declaration that the units are not independently gateable. The per-branch path remains the default for genuinely independent branches. (#55)
